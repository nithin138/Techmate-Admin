...
function ProductsHeader() {
  ...
  const getPendingProductData = async () => {
    ...
    try {
      await axios
        .get(`${baseURL}/api/blog/stats`, {
          headers: {
            Authorization: `${token}`,
          },
        })
        .then((response) => {
          //console.log(response, "response from blog header");
          if (response.status === 200) {
            // Ensure response.data is an array before accessing length
            setTotalCount(response?.data?.totalBlogs )
            setApprovedCount(response?.data?.publishedBlogs);
            }
        });
    }
     catch (error) {
        ...
        <CardBody>
          <div className="d-flex align-items-center justify-content-between">
            <div className="d-flex align-items-center gap-2">
              ...
              <h6 className="mb-0">Total Blogs</h6>
            </div>
          </div>
      ...
      <Col xl="3" sm="6">
        ...
        <CardBody>
          <div className="d-flex align-items-center justify-content-between">
            <div className="d-flex align-items-center gap-2">
              <img src={square} alt="" className="square_box" />
              <h6 className="mb-0">Published Blogs</h6>
            </div>
          </div>
          <div className="d-flex justify-content-between mt-3">
